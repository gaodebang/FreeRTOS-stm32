...
/*                                        FreeRTOS基础配置配置选项                                              */
/***************************************************************************************************************/
#define configUSE_PREEMPTION					1                       //为1时使用抢占式内核，为0时使用协程
#define configUSE_TIME_SLICING					1						//为1时使能时间片调度(默认是使能的)

#define configUSE_PORT_OPTIMISED_TASK_SELECTION	1                       //为1时启用特殊方法来选择下一个要运行的任务，一般是硬件计算前导零指令，如果所使用的MCU没有这些硬件指令的话此宏应该设置为0！
#define configUSE_TICKLESS_IDLE					0                       //为1时启用低功耗tickless模式
...
#define configUSE_MALLOC_FAILED_HOOK			0                       //为1时使用内存申请失败钩子函数
#define configUSE_APPLICATION_TASK_TAG			0                       //为1时使用任务标签值的功能，操作系统不使用该值
